# ==================================================
# File: inbound.py
# Path: app/clients/klr_demo/inbound.py
...
from app.clients.klr_demo.customer.booking_service import handle_booking_command
from app.clients.klr_demo.customer.main_menu_service import handle_main_menu
from app.clients.klr_demo.customer.menu_service import handle_menu_command
...
from app.clients.klr_demo.admin.admin_router import route_admin_message

logger = logging.getLogger("klr_demo.inbound")
...
def handle_klr_demo_inbound(
    ...
    try:
        ...
        if db.execute(
            text("SELECT 1 FROM r_klr_demo__staff WHERE msisdn = :phone AND role = 'admin' LIMIT 1"),
            {"phone": sender_msisdn},
        ).fetchone():
            role = "admin"

        elif db.execute(
            text("SELECT 1 FROM r_klr_demo__staff WHERE msisdn = :phone LIMIT 1"),
            {"phone": sender_msisdn},
        ).fetchone():
            role = "staff"
        ...
        result = db.execute(
            text(
                """
                INSERT INTO r_klr_demo__customers (phone)
                VALUES (:phone)
